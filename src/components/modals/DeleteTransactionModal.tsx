...
const DeleteTransactionModal = ({
  ...
  const handleConfirm = async () => {
    ...
    try {
      await handleDeleteConfirm(transactionId)
      setIsSuccess(true)
      startTimeout()
    } catch {
  ...
  return (
    <ModalTemplate isOpen={!!transactionId} onClose={onClose}>
      {isSuccess && <SuccessMessage />}
      {!isSuccess && (
        <>
          <DialogTitle as="h1" className="text-xl font-bold text-gray-800 dark:text-gray-100">
