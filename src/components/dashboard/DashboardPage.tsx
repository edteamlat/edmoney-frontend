...
const DashboardContent = () => {
  ...
  const { data: recentTransactions = [], isLoading: isRecentTransactionsLoading } = useQuery({
    queryKey: ["RECENT_TRANSACTIONS"],
    queryFn: () => transactionsService.getRecentTransactions(user?.id || ""),
    enabled: !!user,
  })
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const summaryData = await transactionsService.getSummary(user.id)
  ...
  if (isLoading || isUserLoading || isRecentTransactionsLoading) {
    return (
